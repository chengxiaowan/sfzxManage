	...
	<div class="wrapper wrapper-content animated fadeInUp">
		...
		<div class="ibox-content">
			<div class="row">
				<div class="col-xs-12">
					<!-- 检索  -->
					<form class="form-inline" role="form" action="workBench/myCustomerList.do" method="post" name="customerForm" id="customerForm">
						<div class="form-group">
							<input id="nav-search-input" name="keywords" placeholder="客户名称,合同销售,<c:if test="${empty pd.saleId }">订单销售,</c:if>所属省,所属市"  style="width: 398px" value="${pd.keywords }" class="form-control" type="text">
						</div>
			...
			<table id="simple-table" class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<c:if test="${empty pd.saleId }"><th class="center">订单销售人</th></c:if>
							...
							<c:if test="${QX.cha == 1 }">
								<c:forEach items="${dataList}" var="list" varStatus="vs">
									<tr>
										...
										<td class="center"><a title="点击查看详情" onclick="viewCustomer('${list.id}')" style="cursor: pointer;">${list.name }</a></td>
										<td class="center">${list.contractSaleName }</td>
										<c:if test="${empty pd.saleId }"><td class="center">${list.orderSaleName }</td></c:if>
