...
(function ($) {
	...
	$.fn.ajaxChosen = function (ajaxOptions, options, chosenOptions) {
		...
		ajaxOptions.success = function (data, textStatus, jqXHR) {
			...
			keyRight.which = 39;
			//highlight
			input.trigger(keyRight).get(0).style.background = inputBG;
			input.val(!inputEmptied ? inputEmptiedValue : "" );
			select.trigger("chosen:winnow");
			if (items.length > 0) {
    ...
    function processValue(e) {
        ...
        (e.which && (
			...
			if(options.external){
				ajaxOptions.data.ROLE_ID = options.external;
				$("#orderId").val()&&(ajaxOptions.data.orderId = $("#orderId").val())
			}
			if(options.external2){
				ajaxOptions.data.flags = options.external2.flags;
				ajaxOptions.data.orderId = $("#orderId").val();
			}
			//show loading
