	...
	<div class="wrapper wrapper-content animated fadeInUp">
		...
		<div class="ibox-content">
			<div class="row">
				<div class="col-xs-12">
					<!-- 检索  -->
					<form class="form-inline" role="form" action="${pageContext.request.contextPath}${pd.action }?s=${pd.s}" method="post" name="customerForm" id="customerForm">
						...
						<c:when test="${not empty dataList}">
							<c:if test="${QX.cha == 1 }">
								<c:forEach items="${dataList}" var="list" varStatus="vs">
									<tr class="bg-limit-time1">
										<td class='center' style="width: 30px;">${vs.index+1}<input class="list-item-hook hide" type='checkbox' name='ids' data-ctime="${list.createTime }" data-lastreport="${list.reportTime }" data-status="${list.status }" value="${list.id }" /></td>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/js/lib/colorChange.js"></script>
<script>
