package com.yocto.controller.system.head;
...
import com.yocto.util.SmsUtil;
import com.yocto.util.Tools;
import com.yocto.util.Watermark;
import com.yocto.util.mail.SimpleMailSender;

/**
 * 类名称：HeadController 创建人： 修改时间：2015年11月23日
 * 
 * @version
 */
@Controller
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys2() throws Exception {
		...
		Tools.writeFile(Const.FWATERM,
				pd.getString("isCheck1") + ",fh," + pd.getString("fcontent") + ",fh," + pd.getString("fontSize") + ",fh," + pd.getString("fontX") + ",fh," + pd.getString("fontY")); // 文字水印配置
	...
	public ModelAndView saveSys3() throws Exception {
		...
		Tools.writeFile(Const.WEBSOCKET,
				pd.getString("WIMIP") + ",fh," + pd.getString("WIMPORT") + ",fh," + pd.getString("OLIP") + ",fh," + pd.getString("OLPORT") + ",fh," + pd.getString("FHsmsSound")); // websocket配置
		mv.addObject("msg", "OK");
		mv.setViewName("save_result");
		return mv;
	}
