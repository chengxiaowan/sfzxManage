				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form class="form-inline" action="dictionaries/list.do" method="post" name="Form" id="Form">
							<div class="form-group nav-search"> 
                                    <input id="keywords" name="keywords" placeholder="请输入关键词" value="${page.pd.keywords }" class="form-control nav-search-input" type="text">
                                </div>
                                <div class="form-group">
                                	<select class="form-control" name="DICTIONARIES_ID" id="DICTIONARIES_ID">
										...
										<c:if test="${QX.cha == 1 }">
											<button type="submit" class="btn btn-primary" onclick="gsearch()"><i class="fa fa-search"></i>  搜索</button>
										</c:if>
 								</form>
								<!-- 检索  -->
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															...
															<td class="center"><c:if test="${QX.edit != 1 && QX.del != 1 }">
																	...
																	<c:if test="${QX.edit == 1 }">
																		<a class="btn btn-success" title="编辑" onclick="edit('${var.DICTIONARIES_ID}');"> <i class="fa fa-pencil-square-o"></i> 编辑
																		</a>
																	</c:if>
																	<c:if test="${QX.del == 1 }">
																		<a class="btn btn-danger" onclick="del('${var.DICTIONARIES_ID}');"> <i class=" fa fa-trash-o"></i> 删除
																		</a>
																	</c:if>
...
<%-- 																			<c:if test="${QX.edit == 1 }"> --%>
<%-- 																				<li><a style="cursor: pointer;" onclick="edit('${var.DICTIONARIES_ID}');" class="tooltip-success" data-rel="tooltip" title="修改"> <span class="green"> <i --%>
<!-- 																							class="ace-icon fa fa-pencil-square-o bigger-120"></i> -->
<!-- 																					</span> -->
<!-- 																				</a></li> -->
<%-- 																			</c:if> --%>
<%-- 																			<c:if test="${QX.del == 1 }"> --%>
<%-- 																				<li><a style="cursor: pointer;" onclick="del('${var.DICTIONARIES_ID}');" class="tooltip-error" data-rel="tooltip" title="删除"> <span class="red"> <i --%>
									...
									<table style="width: 100%;">
										<tr>
											<td style="vertical-align: top;"><c:if test="${QX.add == 1 }">
													<a class="btn btn-sm btn-success" onclick="add('${DICTIONARIES_ID}');"><i class="fa fa-plus"></i> 新增</a>
												</c:if> <c:if test="${null != pd.DICTIONARIES_ID && pd.DICTIONARIES_ID != ''}">
													<a class="btn btn-sm btn-success" onclick="goSondict('${pd.PARENT_ID}');"><i class="fa fa-remove"></i> 返回</a>
												</c:if></td>
			...
			layer.confirm("确定要删除吗?", function(result) {
					...
					$.get(url,function(data){
						...
						if("success" == data.result){
							...
							layer.msg("<span class='bigger-110'>删除失败！请先删除子级或删除占用资源.</span>");
						}else if("false2" == data.result){
							layer.msg("<span class='bigger-110'>删除失败！排查表不存在或其表中没有BIANMA字段.</span>");
						}
					});
			});
		}
