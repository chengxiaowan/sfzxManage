	...
	<div class="wrapper wrapper-content animated fadeInUp">
		...
		<div class="ibox-content">
			<div class="row">
				<div class="col-xs-12">
					<!-- 检索  -->
					<form class="form-inline" role="form" action="showView/lawyer/list.do" method="post" name="customerForm" id="customerForm">
							...
							<c:if test="${QX.cha == 1 }">
								<c:forEach items="${orderList}" var="list" varStatus="vs">
									<tr>
										...
										<td class="center">${list.lawyerName }</td>
										<td class="center"><c:if test="${list.status==0 }">待发送</c:if>
										<c:if test="${list.status==1 }">进行中</c:if>
										<c:if test="${list.status==2 }">已完成</c:if>
										<c:if test="${list.status==3 }">已取消</c:if>
										</td>
