...
<body class="gray-bg">
	<div class="wrapper wrapper-content animated fadeInUp">
		...
		<div class="ibox-content">
			<div class="row">
				<div class="col-xs-12">
					<!-- 检索  -->
					<form class="form-inline" role="form" action="order/list.do" method="post" name="customerForm" id="customerForm">
						<div class="form-group">
							<input id="nav-search-input" name="keywords" style="width: 398px" placeholder="运作人、销售人、债权人(公司)、债务人(公司)" value="${pd.keywords }" class="form-control" type="text">
						</div>
						<div class="form-group">
							案件类型 
						...
						<div class="form-group">
							案件状态 <select id="status1" name="status1" class="form-control" style="width: 100px;" data-placeholder="案件状态">
						...
						<c:if test="${(isYunzuoAdmin||isKefu) &&(!isKefu1) }">
							<a class="btn btn-danger" id="btn-distribution" onclick="run(this);" disabled> <i class="fa fa-send-o" title="运作分配"></i> 分配运作人
							</a>
						</c:if>
						
							<a class="btn btn-danger" id="btn-placeon" onclick="total(this,8);"> <i class="fa fa-folder-o" title="归档"></i> 归档
							</a>
							...
							<c:if test="${QX.cha == 1 }">
								<c:forEach items="${orderList}" var="list" varStatus="vs">
									<tr>
										...
										<td class="center"><c:if test="${list.isFiled==1 }">是</c:if><c:if test="${list.isFiled==0 }">否</c:if></td>
										<%-- <td class="center">${fn:substring(list.createTime, 0, 19)}</td> --%>
										<td class="center">${list.saleName}</td>
										<td class="center">
	...
	<div class="" style="height:0;overflow:hidden">
					<div class="row">
						<div class="col-sm-12 form form-horizontal" id="teshuBox">

							<div class="form-group" style="margin-top: 20px;">
								<label class="col-sm-3 control-label"><font color="red">*</font><span style="font-weight: 600;">运作：</span></label>
								<div class="col-sm-9">
									<select class="form-control col-sm-12 htz-sale" id="tssalechange" name="saleName1" style="width: 220px">
										<option value="">请选择运作人</option>
									</select>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	var name = decodeURI(yo.getQueryString('name'));
	if(name){
		$("input[name=keywords]").val(name);
	}
	var roleId = "02178e62f17b4926bb7014f3ad5a1ebe,de9de2f006e145a29d52dfadda295353,b693f826af0545b5a1c60447a27c3089";
	select_init('[name=saleName1]', '请选择员工', 1,)
	
	function select_init(el, placeholder, clearbtn, ) {
				const that = this;
				return $(el).select2({
	...
	//分配运作
	function run(dom){
		$("#tssalechange").val(null).trigger('change')
		var id = [];
		var runId = [];
		$.each(checks,function(){
			id.push(this.id)
	...
	//	var status = $checked.data("status");
		...
		var title = '分配运作人';
		var ROLE_ID = "02178e62f17b4926bb7014f3ad5a1ebe，de9de2f006e145a29d52dfadda295353,b693f826af0545b5a1c60447a27c3089";
...
/* 		if(status==3 ||status ==4){
		...
		console.log(id)
		if(id.length<=1){
		var info = {id:runId[0],name:checks[0].runName};
...
//			});
			var dialog = layer.open({
					...
					btnAlign: 'c',
					yes() {
						var runnerId = $('[name=saleName1]').val();
						if(runnerId == ''){
							layer.msg("请选择运作人")
							return false
						}
						console.log(runnerId);
						var url = "/order/doRun.do?id="+ id.join(",") + "&runnerId=" + runnerId;
						$.get(url, function(data) {
								if(data.error=="00"){
									layer.msg('分配成功')
									layer.close(dialog)
		...
		//订单选择/指可选1
		 function checkAuthority (){
			...
			if(!checks.length){
				...
				btnPlaceOn.removeClass('btn-primary').addClass("btn-danger");
				btnDistribution.removeClass('btn-primary').addClass("btn-danger");
				return;
			}else{
				...
				btnPlaceOn.removeClass('btn-danger').addClass("btn-primary");
				btnDistribution.removeClass('btn-danger').addClass("btn-primary");
			}
