!function ($) {
    ...
    Pagination.prototype = {
        ...
        _draw: function () {
            var tpl = '<form onsubmit="return false;" class="sui-pagination form form-inline';
        ...
        _drawInner: function () {
            ...
            var tpl = '<ul class="pagination pull-left" style="margin:0">' + '<li class="prev' + (this.currentPage - 1 <= 0 ? ' disabled' : ' ') + '"><a href="#" data="' + (this.currentPage - 1) + '">«上一页</a></li>';
        ...
        //值传递
        _drawCtrl: function () {
            var tpl = '<div class="pull-left form-group form-group-sm">&nbsp;' + (this.displayInfoType == 'itemsCount'? '<span>共' + this.itemsCount + '条</span>&nbsp;' :'<span>共' + this.pages + '页</span>&nbsp;') + 
            '<span>' + '&nbsp;到&nbsp;' + '<input type="text" style="display:inline-block;width:70px" class="page-num form-control"/><button class="page-confirm btn btn-white btn-sm">确定</button>' + '&nbsp;页' + '</span>' + '</div>';
            return tpl;
        },
