/*! qiniu-js-sdk v1.0.13-beta | Copyright 2015 by Qiniu */ ! function(global) {
	...
	function QiniuJsSDK() {
		...
		function log(a, b) {
			...
			if (that.detectIEVersion()) {
				...
				console.log(d)
			} else b.unshift(c), console.log.apply(console, b)
		}
		...
		this.resetUploadUrl = function() {
			...
			var c = function() {
				if (a.uptoken) that.token = a.uptoken;
					...
					var b = that.createAjax();
					b.open("GET", that.uptoken_url, !0), b.onreadystatechange = function() {
						if (4 === b.readyState && 200 === b.status) {
							var a = that.parseJSON(b.responseText);
			...
			logger.debug("new plupload.Uploader(option)"), j.bind("Init", function(b, d) {
				logger.debug("Init event activated"), a.get_new_uptoken || c(), c()
				...
				c = c || a.settings && a.settings.auto_start, logger.debug("auto_start: ", c), logger.debug("files: ", b), c && setTimeout(function() {
					a.start(), logger.debug("invoke up.start()")
				}, 0), a.refresh()
			}), logger.debug("bind FilesAdded event"), j.bind("BeforeUpload", function(b, e) {
				logger.debug("BeforeUpload event activated"), e.speed = e.speed || 0, h = "", a.get_new_uptoken && c();
